...
const TotalAttendance = require('../models/totalAttendance.model.js')
...
exports.submitTodaysAtendance = (req, res) => {
    ...
    totalAttendanceSchema.save().then(data => {
        res.send("Success");
    }).catch(err => {
        res.status(500).send(
            {
...
exports.getTotalAttendanceForClass = (req, res) => {
    TotalAttendance.find({ classId: req.params.classId }).then(students => {
        res.send(students);
    }).catch(err => {
        res.status(500).send({
            message: "Server Error" || err
